...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickDeleteToDo(View view) {
        ...
        String[] selectionArgs = {String.valueOf(activeTodo)};
        int rowsDeleted = getContentResolver().delete(
                TodoProvider.CONTENT_URI,   // the user dictionary content URI
                "_id = ?",                   // the column to select on
                selectionArgs                     // the value to compare to
        );

        if (rowsDeleted > 0) {
            Toast.makeText(getBaseContext(),
                    "Deleted todo with ID " + rowsDeleted, Toast.LENGTH_SHORT).show();
            getTodos();
            todoText.setText("");
        } else {
            Toast.makeText(getBaseContext(),
                    "There may have been an issue deleting the record", Toast.LENGTH_SHORT).show();
        }
        activeTodo = -1;
        changeState();
    }
